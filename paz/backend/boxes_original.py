...
def apply_non_max_suppression(boxes, scores, iou_thresh=0.45, top_k=200):
    ...
    while len(remaining_sorted_box_indices) > 0:
        ...
        remaining_sorted_box_indices = remaining_sorted_box_indices[intersec_over_union_mask]

    return selected_indices.astype(int), num_selected_boxes


def nms_per_class(box_data, nms_thresh=0.45, epsilon=0.01, top_k=200):
    ...
    for class_arg in range(num_classes):
        nms_boxes, class_labels = _nms_per_class(nms_boxes, class_labels, class_arg, *args)
    return nms_boxes, class_labels


def _nms_per_class(
    nms_boxes, class_labels, class_arg, decoded_boxes, class_predictions, epsilon, nms_thresh, top_k
):
...
def merge_nms_box_with_class(box_data, class_labels):
    ...
    decoded_boxes = box_data[:, :4]
    class_predictions = box_data[:, 4:]
    retained_class_score = suppress_other_class_scores(class_predictions, class_labels)
